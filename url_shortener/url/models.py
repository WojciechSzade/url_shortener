...
def validate_url_link(value):
    if not validate_url_prefix(value):
        raise ValidationError("Url must start with http://, https://, ftp:// or ftps://")
...
class Url(models.Model):
    ...
    original_url = models.CharField(max_length=250, blank=False, help_text="Enter the URL you want to shorten", validators=[validate_url_text, MinLengthValidator(2)],verbose_name="Original URL text" )
    original_url_link = models.URLField(max_length=250, null=True, help_text="original_url with protocol if needed", validators=[validate_url_link, URLValidator, MinLengthValidator(2)],verbose_name="Original URL url" )
    shorten_url = models.SlugField(max_length=shorten_url_length, primary_key=True, validators=[validate_slug], verbose_name="Shorten URL slug")
    pub_date = models.DateTimeField( default=timezone.now, validators=[validate_date], verbose_name="Publication date")
    last_access = models.DateTimeField(default=timezone.now, validators=[validate_date], verbose_name="Last access date")
